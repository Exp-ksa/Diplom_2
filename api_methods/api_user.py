...
class User:
    ...
    def create_user(payload):
        return requests.post(Url.CREATE_USER_ENDPOINT, json=payload, verify=False)

    @staticmethod
    @allure.step('Авторизация пользователя с данными: {payload}')
    def login_user(payload):
        return requests.post(Url.AUTHORIZATION_ENDPOINT, json=payload, verify=False)   

    @staticmethod
    @allure.step('Выход из системы пользователя с данными: {payload}')
    def logout_user(payload):
        return requests.post(Url.LOGOUT_USER_ENDPOINT, json=payload, verify=False)
